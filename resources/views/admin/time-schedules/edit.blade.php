<x-layouts.app :title="__('Edit Time Schedule')">
    <div class="mb-8">
        ...
        <flux:heading size="xl" level="1">Edit Time Schedule</flux:heading>
        <flux:text class="mb-6 mt-2 text-base">Update the time schedule for {{ $timeSchedule->subject->name }}</flux:text>
    </div>
    ...
    <flux:card>
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('admin.time-schedules.update', $timeSchedule) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <flux:field>
                    ...
                    <flux:select name="subject_teacher" id="subject_teacher" required>
                        ...
                        @foreach($subjectTeacherOptions as $option)
                            @php
                                $currentValue = $timeSchedule->subject_id . '_' . ($timeSchedule->teacher_id ?? 'null');
                                $optionValue = $option['subject_id'] . '_' . ($option['teacher_id'] ?? 'null');
                            @endphp
                            <option value="{{ $optionValue }}" {{ (old('subject_teacher', $currentValue) == $optionValue) ? 'selected' : '' }}>
                                {{ $option['display_name'] }}
                            </option>
                        @endforeach
                    </flux:select>
                ...
                <input type="hidden" name="subject_id" id="subject_id" value="{{ old('subject_id', $timeSchedule->subject_id) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <flux:field>
                        ...
                        <flux:select name="day_of_week" id="day_of_week" required>
                            ...
                            @foreach($days as $key => $value)
                                <option value="{{ $key }}" {{ (old('day_of_week', $timeSchedule->day_of_week) === $key) ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </flux:select>
                    ...
                    <flux:field>
                        ...
                        <flux:input 
                            ...
                            name="start_time" 
                            id="start_time" 
                            value="{{ old('start_time', $timeSchedule->start_time->format('H:i')) }}" 
                            required 
                        />
                        @error('start_time')
                            <flux:error>{{ $message }}</flux:error>
                    ...
                    <flux:field>
                        ...
                        <flux:input 
                            ...
                            name="end_time" 
                            id="end_time" 
                            value="{{ old('end_time', $timeSchedule->end_time->format('H:i')) }}" 
                            required 
                        />
                        @error('end_time')
                            <flux:error>{{ $message }}</flux:error>
                ...
                <flux:field>
                    ...
                    <flux:input 
                        ...
                        name="room" 
                        id="room" 
                        value="{{ old('room', $timeSchedule->room) }}" 
                ...
                <flux:field>
                    ...
                    <flux:textarea 
                        ...
                        maxlength="500"
                    >{{ old('notes', $timeSchedule->notes) }}</flux:textarea>
                    @error('notes')
                        <flux:error>{{ $message }}</flux:error>
                    @enderror
                </flux:field>
